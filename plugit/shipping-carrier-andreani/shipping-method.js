...
const AndreaniCarrier = require('./andreani-carrier')
const { db } = require('#/express')

const _carriers = {
	andreani: AndreaniCarrier,
}
...
class ShippingMethod extends DeliveryMethod {
	...
	static async calculate(orderMng, zipcodeId) {
		...
		const zipCode = await AddressService.getZipcodeById(zipcodeId)
		const { code: postalCode } = zipCode
		...
		for (let rateConfig of rateConfigs) {
			...
			let result = await carrier.calculate(rateKey, postalCode)
			if (result !== false) {
...
ShippingMethod.registerCarrier(AndreaniCarrier)
